...
def render_gallery_card(claim_data, claim_key): # claim_key is important for unique widget keys
    ...
    with st.container(border=True): # Simple card look
        ...
        if thumbnail_url:
            ...
            st.image(thumbnail_url, use_column_width=True, caption="") # Reverted to use_column_width
        else:
...
def render_claim_details(claim_data):
    ...
    if thumbnail_url:
        st.image(thumbnail_url, caption=publisher_name_for_alt, use_column_width=True) # Reverted to use_column_width
    else:
